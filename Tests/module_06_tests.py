...
# TODO: all biomarker tests should check fix/solver combinations
class Module06Tests(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        Necessary because module_06 modifies object in place.
        """
    ...
    def test_object_modified_in_place(self):
        output_object = module_06.run_module(self.dwi_input, solver='wls', fix_method='abs')
    ...
    def test_input_other_fields_not_changed(self):
        ...
        self.assertTrue(
            all(
                [np.array_equal(input_attributes[key], getattr(output_object, key)) for key in input_attributes.keys()]
            )
        )

    """
    OUTPUT Tests
    - testing whether output attribute (biomarkers) has expected properties for each input parameter combination.
    """
