function  [MapW]=sensitivity_map(Sz,Ncoil,scale)
...
% generate real sensitivity maps
if(scale == -1)
    MapW=MapW./sqrt(Ncoil/2);	

% generate both real and imag sensitivity maps    
else
    MapW=(sqrt(scale).*MapW + 1i.*sqrt(1-scale).*MapW)./sqrt(Ncoil/2);	    
end
